...
class image
{
    ...
    */
    ...
    public static function thumb($sourcefile, $destfile, $forcedwidth = 80, $forcedheight = 80)
    {
        ...
        $tmpfile = UPLOADPATH . md5($destfile);
        switch ($destext) {
            case 'jpg':
                imagejpeg($img_dst, $tmpfile, $imgcomp);
                break;
            case 'gif':
                imagegif($img_dst, $tmpfile, $imgcomp);
                break;
            case 'png':
                imagepng($img_dst, $tmpfile, version_compare(PHP_VERSION, '5.1.2') == 1 ? 7 : 70);
                break;
        }
