...
class medoo
{
    ...
    protected $database_name = 'lcglcg';
    ...
    {
	//   echo '<br/>';echo str_replace('"', '`',$query);
	if (isset($_GET['debug']) && DEBUG)
	    echo str_replace('"', '`', $query) . PHP_EOL;
	...
	foreach ($data as $key => $value) {
		...
		if (isset($match[4])) {
			...
			switch ($type) {
				...
				{
				    $t1 = microtime(true);
				    //isset($_GET['debug']) && echo $this->select_context($table, $join, $columns, $where);
				    $query = $this->query($this->select_context($table, $join, $columns, $where));
				    $result = $query ? $query->fetchAll((is_string($columns) && $columns != '*') ? PDO::FETCH_COLUMN : PDO::FETCH_ASSOC) : false;
				    $sql = str_replace('"', '`', $this->select_context($table, $join, $columns, $where));
				    DEBUG == 1 && isset($_SERVER['sqls']) && count($_SERVER['sqls']) < 1000 && $_SERVER['sqls'][] = htmlspecialchars(stripslashes(number_format(microtime(1) - $t1, 4) . '秒        ' . $sql));
				    return $result;
				}
				...
				public function insert($table, $datas)
				{
					...
					// if(isset($_GET['debug'])){echo 'INSERT INTO "' . $table . '" (' . implode(', ', $columns) . ') VALUES (' . implode($values, ', ') . ')'.PHP_EOL;exit;}
					// echo str_replace('"','`','INSERT INTO "' . $table . '" (' . implode(', ', $columns) . ') VALUES (' . implode($values, ', ') . ')'.PHP_EOL);die();
					$this->exec('INSERT INTO "' . $table . '" (' . implode(', ', $columns) . ') VALUES (' . implode($values, ', ') . ')');
						...
						//echo str_replace('"','`','UPDATE "' . $table . '" SET ' . implode(', ', $fields) . $this->where_clause($where).PHP_EOL);//die();
							...
							$replace_query[] = $column . ' = REPLACE(' . $this->column_quote($column) . ', ' . $this->quote($replace_search) . ', ' . $this->quote($replace_replacement) . ')';
								...
								$replace_query[] = $columns . ' = REPLACE(' . $this->column_quote($columns) . ', ' . $this->quote($replace_search) . ', ' . $this->quote($replace_replacement) . ')';
									...
									return $this->query('SELECT EXISTS(' . $this->select_context($table, $join, $column, $where, 1) . ')')->fetchColumn() === '1';
									    ...
									    {
									    //print_r(str_replace('"','`',$this->select_context($table, $join, $column, $where, 'COUNT')));
									    if (isset($_GET['debug']) && DEBUG)
									    print_r(str_replace('"','`',$this->select_context($table, $join, $column, $where, 'COUNT')));
