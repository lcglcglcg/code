...
{
    char path[128];
    sprintf(path, "%s/%s", SITE_BUILDER_HOME, site_builder_uuid(key));
    if (mkdir(path, 0755))
	return -1;
...
int site_builder_copy(const char *dest_path, const char *src_path)
{
	...
	int size = fread(buffer, 1, 2048, src_fp);
	if (size <= 0) break;
...
int site_builder_load_dir(const char *path, int path_len, const char *template_id)
{
	...
	if (dir_info->d_type == 4) { //dir
	    ...
	    sprintf(dest_path, "%s/%s/%s", SITE_BUILDER_HOME, template_id, dir_info->d_name);
	    ...
	    sprintf(dest_path, "%s/%s%s", SITE_BUILDER_HOME, template_id, key);
