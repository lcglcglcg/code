...
int lg_network_epoll_event_add(lg_network_epoll_t *ep, int sockfd, int mask)
{
    ...
    ep->event_ctl.events = mask ? mask: EPOLLIN | EPOLLET;
...
void lg_network_epoll_addr_add(lg_network_epoll_t *ep, int sockfd, struct sockaddr_in addr)
{

    if (sockfd > 0 && sockfd < ep->event_max) {

	ep->addr_pool[sockfd] = addr;
    }
}

void lg_network_epoll_addr_get(lg_network_epoll_t *ep, int sockfd, char *addr_str)
{
    if (sockfd > 0 && sockfd < ep->event_max) {
...
int lg_network_epoll(lg_network_epoll_t *ep)
{
	...
	//fprintf(stdout, "prt=%p\n", ep->event_pool[n].data.ptr);
