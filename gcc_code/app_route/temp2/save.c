...
int sig_handle(int sig, void (*sigcall)(int))
{
...
int get_ip_port(char *str, char *ip, int *port)
{
    char *p;
...
void compose_addr(struct sockaddr_in *addr, char *ip, int port)
{
...
void map_item_enqueue(struct map_list_head *h, struct map_list *item)
{
    if (!h->head)
	h->head = h->tail = item;
    else
...
void parse_addr_str(struct map_list_head *h, char *saddr, char *daddr)
{
...
void output_map_list(struct map_list_head *h)
{
    struct map_list *item;
    char ip[16];
...
int init_map_list(struct map_list_head *h)
{
    FILE *fp;
