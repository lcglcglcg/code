 ...
 * 
 * 
 * 用来读取QQwry.dat文件，以根据ip获得好友位置，QQwry.dat的格式是 一. 文件头，共8字节 1. 第一个起始IP的绝对偏移， 4字节 2.
 * 最后一个起始IP的绝对偏移， 4字节 二. "结束地址/国家/区域"记录区 四字节ip地址后跟的每一条记录分成两个部分 1. 国家记录 2. 地区记录
 * 但是地区记录是不一定有的。而且国家记录和地区记录都有两种形式 1. 以0结束的字符串 2. 4个字节，一个字节可能为0x1或0x2 a.
...
public class IPSeeker {
	...
	private static final String IP_FILE = "ip.dat";
	// 一些固定常量，比如记录长度等等
	...
	private static IPSeeker instanse=null;
	public synchronized static IPSeeker getInstanse(){
		if(instanse==null){
			instanse=new IPSeeker();
		}
		return instanse;
	}
	/**
