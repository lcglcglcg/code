...
const char *MONGO_HOST = "mongodb://127.0.0.1:27017";
extern char *strptime(const char *s, const char *format, struct tm *tm);
int ad_system_mongo_init(lg_list_t *config_file_head)
{

    //if (!(MONGO_HOST = lg_config_file_find(config_file_head, "mongodb_host")))
    //return -1;
    return 0;
}
...
int ad_system_mongo_conn(ad_system_mongo_t *p)
{
    ...
    if (!(p->client = mongoc_client_new (MONGO_HOST)))
	return -1;
...
char *ad_system_mongo_time_to_string_week(time_t date_time, char *buffer)
{
    ...
    struct tm date_tm_s = {0};
    struct tm date_tm_e = {0};
...
int ad_system_mongo_to_number(bson_iter_t *iter)
{
	...
	case 0x01://BSON_TYPE_DOUBLE
	    n = bson_iter_double(iter);
	    break;

	case 0x10://BSON_TYPE_INT32
	    n = bson_iter_int32(iter);
	    break;

	case 0x12://BSON_TYPE_INT64
...
//标准报告
int ad_system_mongo_report_standard_element(lg_list_t *head, const bson_t *doc, int time_type)
{
    //fprintf(stdout, "%s\n", bson_as_json(doc, NULL));

    bson_iter_t iter_key;
    if (!bson_iter_init_find(&iter_key, doc, "_id"))
	return -1;

    bson_iter_t iter_value;
    if (!bson_iter_init_find(&iter_value, doc, "value"))
	return -1;

    uint32_t key_len = 0;
    const uint8_t *key_data = NULL;
    bson_iter_document(&iter_key, &key_len, &key_data);

    uint32_t value_len = 0;
    const uint8_t *value_data = NULL;
    bson_iter_document(&iter_value, &value_len, &value_data);

    bson_t key;
    if (!bson_init_static (&key, key_data, key_len))
	return -1;

    bson_t value;
    if (!bson_init_static (&value, value_data, value_len))
	return -1;

    //fprintf(stdout, "[key]%s\n", bson_as_json(&key, NULL));
    //fprintf(stdout, "[value]%s\n", bson_as_json(&value, NULL));

    bson_iter_t iter_campaign_id;
    if (!bson_iter_init_find(&iter_campaign_id, &key, "campaign_id"))
	return -1;

    bson_iter_t iter_group_id;
    if (!bson_iter_init_find(&iter_group_id, &key, "group_id"))
	return -1;

    bson_iter_t iter_creative_id;
    if (!bson_iter_init_find(&iter_creative_id, &key, "creative_id"))
	return -1;

    bson_iter_t iter_time;
    if (!bson_iter_init_find(&iter_time, &key, "time"))
	return -1;

    bson_iter_t iter_impression;
    if (!bson_iter_init_find(&iter_impression, &value, "impression"))
	return -1;

    bson_iter_t iter_click;
    if (!bson_iter_init_find(&iter_click, &value, "click"))
	return -1;

    bson_iter_t iter_cost;
    if (!bson_iter_init_find(&iter_cost, &value, "cost"))
	return -1;
    ...
    return ad_system_report_standard_add(head,
	    ad_system_mongo_time_to_string(bson_iter_time_t(&iter_time), time_buffer, time_type),
	    bson_iter_utf8(&iter_campaign_id, NULL),
	    bson_iter_utf8(&iter_group_id, NULL),
	    bson_iter_utf8(&iter_creative_id, NULL),
	    impression,
	    click,
	    cost);
}
	...
	int display_style, int bidding_model, const char *account_id, const char *time_s, const char *time_e,int time_type)
{
    ...
    bson_t query;
    bson_init (&query);
    bson_append_int64(&query, "_id.display_style", -1, display_style);
    bson_append_int64(&query, "_id.bidding_model", -1, bidding_model);
    bson_append_utf8(&query, "_id.account_id", -1, account_id, -1);
    bson_append_document(&query, "_id.time", -1, &query_time);
	...
	if (mongoc_cursor_next(conn.cursor, &conn.doc)) {
	    ...
	    ad_system_mongo_report_standard_element(head, conn.doc, time_type);
	}
    }
...
int ad_system_mongo_report_region_element(lg_list_t *head, const bson_t *doc, int time_type)
{
    ...
    if (!bson_iter_init_find(&iter_region_code, &key, "region_code"))
	return -1;
    ...
    if ((region_str = bson_iter_utf8(&iter_region_code, NULL)))
	region_code = atoi(region_str);
...
int ad_system_mongo_stat_load(ad_system_mongo_stat_t *p, const char *table, int display_style, const char *account_id)
{
	...
	if (mongoc_cursor_next(conn.cursor, &conn.doc)) {
	    ...
	    bson_iter_t iter_value;
	    if (!bson_iter_init_find(&iter_value, conn.doc, "value"))
		return -1;
...
int ad_system_mongo_stat_list_element(lg_list_t *head, const bson_t *doc, const char *name)
{
    ...
    bson_iter_t iter_name;
    if (!bson_iter_init_find(&iter_name, &key, name))
	return -1;
...
int mongo_main()
{
    ...
    lg_list_t head;
    ad_system_mongo_stat_list_load(&head, CAMPAIGN_DAY_TEMP, 1, "824", "campaign_id");
