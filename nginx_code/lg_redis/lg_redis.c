...
static void *_lg_redis_malloc(void *pool, size_t size)
{
    return malloc(size);
}

static void _lg_redis_free(void *pool, void *p)
{
    free(p);
}
...
#if 0
static int lg_redis_network_kernel_buffer(int sockfd, int send_size, int recv_size)
{
...
int lg_redis_init(lg_redis_conn_t *conn, char *host, int port)
{
    ...
    conn->memory_pool = NULL;
    conn->malloc = _lg_redis_malloc;
    conn->free = _lg_redis_free;
    return 0;
}
...
int lg_redis_value_info_parse(lg_redis_conn_t *conn, lg_redis_value_buffer_info_t *info, char *buffer)
{
    ...
    info->value_buffer = (char *)conn->malloc(conn->memory_pool, info->value_size + 2);//+2防止越界
    if (!info->value_buffer)
	return -1;

    info->value = (lg_redis_value_t *)conn->malloc(conn->memory_pool, sizeof(lg_redis_value_t));
    if (!info->value) {

	if (conn->free) conn->free(conn->memory_pool, info->value_buffer);
	return -1;
    }
...
int lg_redis_select_base(lg_redis_conn_t *conn, const char *base)
{
...
lg_redis_key_value_t *lg_redis_rand_key_value(lg_redis_conn_t *conn)
{
    ...
    lg_redis_key_value_t *r = (lg_redis_key_value_t *)conn->malloc(conn->memory_pool, sizeof(lg_redis_key_value_t));
    if (!r) return NULL;
