	...
	site_builder_page_info_t *page_info)
{
    ...
    site_builder_element_tree_t *element = site_builder_element_key_find(&loc_conf->affiliated_file, uri->key);
...
int site_builder_filter(ngx_http_request_t *r, site_builder_loc_conf_t *loc_conf)
{
    ...
    // URI parse
    if (site_builder_uri_parse(r, loc_conf->location_name, loc_conf->location_name_len, &page_info->uri))
