	...
	site_builder_page_info_t *page_info)
{
    ...
    int i, link_size = 0;
    char rand_buf[20], link_buf[1024];
	...
	if (link_str && element_keyword && link) {
	    ...
	    link->keyword = (char *)element_keyword->buffer;
	    link->keyword_size = element_keyword->buffer_size;

	    link->link = (char *)link_str;
	...
	site_builder_page_info_t *page_info)
{
	...
	// rand 
	site_builder_element_t *element_url = site_builder_element_index_rand(loc_conf->link_external_url);
	site_builder_element_t *element_keyword = site_builder_element_index_rand(loc_conf->link_external_keyword);

	if (element_url && element_keyword) {

	    lg_string_rand_number(rand_buf, rand() % 9 + 4);
	    link_size = sprintf(link_buf, "%s/%s.html", element_url->buffer, rand_buf);
	...
	site_builder_page_info_t *page_info)
{
	...
	if (element_group && element_keyword) {
	    ...
	    lg_string_rand_keyword(rand_buf, rand() % 7 + 2);
	    // lg_string_rand_number(rand_buf2, rand() % 7 + 4);
	    // link_size = sprintf(link_buf, "%s.%s/%s.html", rand_buf, element_group->buffer, rand_buf2);
