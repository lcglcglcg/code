...
char *site_builder_init(ngx_conf_t *cf, site_builder_loc_conf_t *loc_conf)
{
    ...
    if (loc_conf->template_dir.len == 0
	    ...
	    || loc_conf->link_external_url_file.len == 0
	    || loc_conf->link_external_keyword_file.len == 0
    ...
    loc_conf->link_external_url = ngx_pcalloc(cf->pool, sizeof(site_builder_element_head_t));
    loc_conf->link_external_keyword = ngx_pcalloc(cf->pool, sizeof(site_builder_element_head_t));
    ...
    // load link_external url
    if (site_builder_element_load_file(
		(char *)loc_conf->link_external_url_file.data,
		loc_conf->link_external_url,
		cf->pool)) {
    ...
    // load link_external url
    if (site_builder_element_load_file(
		(char *)loc_conf->link_external_keyword_file.data,
		loc_conf->link_external_keyword,
		cf->pool)) {
