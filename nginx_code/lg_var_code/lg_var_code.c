...
int lg_var_code_init(char *symbols)
{
    int i, brect[8];
	...
	if (gdImageStringFT(NULL, &brect[0], 0, "/etc/lg_var_code.ttf", 30, 0, 0, 0, str))
	    return -1;

	int x = brect[2] - brect[6];
	int y = brect[3] - brect[7];

	lg_var_code_font[i].c  = symbols[i];
	lg_var_code_font[i].x  = x;
	lg_var_code_font[i].y  = y;
	...
	x = 0 - brect[6];
	y = 0 - brect[7];
	if (gdImageStringFTEx(im, brect, ft_color, "/etc/lg_var_code.ttf", 30, 0.0, x, y, str, &strex))
	    return -1;

	lg_var_code_font[i].i = im;
    }

    return 0;
}

int lg_var_code_create(lg_var_code_t *p)
{
    int i, j;
    gdImagePtr img, imo;
	...
	if (i > 0) {
	    ...
	    shift=10000;
	    for (sy = 1; sy < lg_var_code_font[j].y; sy++) {
		for (sx = 1; sx < lg_var_code_font[j].x; sx++) {
		    rgb = gdImageTrueColorPixel(lg_var_code_font[j].i, sx, sy);
		    opacity = rgb>>24;
		    if (opacity < 127) {
			left = sx - 1 + x;
			py = sy + y;
	...
	gdImageCopy(img, lg_var_code_font[j].i, x - shift, y, 0, 0, lg_var_code_font[j].x, lg_var_code_font[j].y);

	x += lg_var_code_font[j].x - shift;
	p->key[i] = lg_var_code_font[j].c;
    }
