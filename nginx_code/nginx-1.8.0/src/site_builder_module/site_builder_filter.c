	...
	site_builder_page_info_t *page_info)
{
    ...
    site_builder_element_node_t *element = site_builder_element_key_find(&loc_conf->affiliated_file, uri->affiliated_file);
...
int site_builder_filter(ngx_http_request_t *r, site_builder_loc_conf_t *loc_conf)
{
    ...
    if (page_info->uri.type != SITE_BUILDER_URI_TYPE_HTML)
